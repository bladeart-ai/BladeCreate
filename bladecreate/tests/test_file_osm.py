...
def test_file_osm(
    ...
    # upload files
    osm.upload_object_from_file(
        local_file_path, os.path.join(test_folder_key, "file1")
    )
    osm.upload_object_from_bytes(
        os.path.join(test_folder_key, "file2"), local_file_bytes
    )
    osm.upload_object_from_text(
        os.path.join(test_folder_key, "file3"), local_file_text
    )
    osm.upload_objects_from_folder(
        local_folder_path, os.path.join(test_folder_key, "folder1")
    )
    ...
    # download files
    osm.download_object(
        list(file_keys)[0], os.path.join(tmp_path, "download_1")
    )
    assert os.path.isfile(os.path.join(tmp_path, "download_1"))
    osm.download_objects_from_folder(
        test_folder_key, os.path.join(tmp_path, "download_2/")
    )
